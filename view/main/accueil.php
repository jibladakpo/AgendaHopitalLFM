...
$mois_fr = Array("", "Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Ao�t","Septembre", "Octobre", "Novembre", "D�cembre");
if(isset($_GET['id_praticien']) && isset($_GET['mois']) && isset($_GET['annee']))
{
	$id_praticien=$_GET['id_praticien'];
	$mois=$_GET['mois'];
...
<?php
	for($i=2000;$i<2051;$i++) // l'ann�e va de 1950 � 2500
	{
		echo '<option value="'.$i.'"';
		if($i==$annee)
			echo ' selected ';
...
<?php
	if(in_array($f, $list_dispo)){ // jours de pr�sences
	...
	<?php  if(isset($s->date)){
		echo "<input type='hidden' name='id_patient' value='$da'>$jours_fr[$f] $i <br> $mois_fr[$mois] $annee
		<a href='supprimer_absence.php?id_praticien=$id_praticien&amp;dt=$da&amp;mois=$m&amp;annee=$a' title='Mettre pr�sent'> <img src='image/v.jpg' width='20'/></a>"
		;} ?>
	<table > <!-- invalide location of tag table = c'est voulu sinon probl�me avec le tableau -->
<?php
		$select = $db->query ("SELECT * FROM `agenda_absence`, `agenda_praticien`
				WHERE agenda_praticien.id_praticien = agenda_absence.id_praticien
				AND agenda_absence.id_praticien = $id_praticien
...
<?php if(in_array($f, $list_dispo)){?>
	<?php if($id_praticien == 1 || $id_praticien == 2 || $id_praticien == 7){//2 rendez-vous pour un m�d�cin?>
	...
	<?php }elseif($s->nbheure==0){?>
		...
		<td colspan=2 width="500"  bgcolor="#ffffff"><a href="recherche_patient2.php?action=afficher&amp;id_praticien=<?php echo $id_praticien;?>&amp;dt=<?php echo $da;?>&amp;h=<?php echo $valeur;?>"><img src='image/plus.jpg' width='20'/></a></td>
	<?php }?>
	<?php }else{//un seul rendez-vous pour un m�decin?>
	<?php if($s->nbheure==1){?>
			<!-- rien -->
	<?php }elseif($s->nbheure==0){?>
	<td width='' bgcolor="#dddddd"> <?php echo $valeur ?></td>
